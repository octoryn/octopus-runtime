...
test("the in-memory transport dedupes by idempotency key", async () => {
  const { transport, outbox } = inMemoryTransport();
  const first = await transport.deliver({ to: ["a@x.com"], subject: "s", body: "b" }, { idempotencyKey: "k1" });
  const second = await transport.deliver({ to: ["a@x.com"], subject: "s", body: "b" }, { idempotencyKey: "k1" });
...
test("registering a duplicate workflow id throws", () => {
  ...
  assert.throws(() => runtime.registerWorkflow(planWorkflow({ id: "dup", actions: [] })), ConfigurationError);
});
